...
function App() {
  ...
  function handleChange(e){
    e.preventDefault();
    setId(Number(e.target.value)-1)
  }

  return (
    <>
      <div style={{background:"#646cff",padding:"15px",borderRadius:"20px"}}>
        <select onChange={(event)=>{handleChange(event)}}>
          {pokemons.map((ele,index)=>{
            return <option value={ele.id} key={index}>{ele.name}</option>
          })}
        </select>
        <Display data={pokemons[id]}/>
        <button onClick={(event)=>{prev(event)}} style={{marginRight:"10px"}}>Previous</button>
